...
bool SlamPipeline::init() {
    ...
    params.insert({rtabmap::Parameters::kIcpVoxelSize(), "0.03"});                // 30mm voxel
    params.insert({rtabmap::Parameters::kIcpMaxCorrespondenceDistance(), "0.25"}); // 250mm
    params.insert({rtabmap::Parameters::kIcpIterations(), "40"});
    params.insert({rtabmap::Parameters::kIcpEpsilon(), "0.0005"});
    params.insert({rtabmap::Parameters::kIcpMaxTranslation(), "0.5"});            // reject jumps > 500mm
    params.insert({rtabmap::Parameters::kIcpMaxRotation(), "0.3"});               // reject jumps > ~17 degrees
