...
| `color_mode` | string | `"intensity"` | How to color map points. `"intensity"`: grayscale from reflectivity. `"flat"`: solid color from `map_color`. `"age"`: blue (old) → red (new) gradient to visualize convergence — overlapping colors mean good alignment, smearing means drift |
